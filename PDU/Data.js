...
Data.prototype._checkData = function()
{
    ...
    for(var i = 0; i < this._data.length; i++){
        // get byte
        var char = this._data.substr(i, 1);
        var byte = Helper.order(char);

        if(byte > 127){ //(byte > 0xC0)
            //bugfix: check if char is not in the gsm7 alfabeth
            if (Helper.ALPHABET_7BIT.indexOf(char) == -1)
            {
